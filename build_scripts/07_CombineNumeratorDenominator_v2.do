...
use builds/cohort_prevalent_copd_stratvars_value_denom_eligible, clear


count

summarize copd_prevalent_first copd_incident_first pr_eligible pr_considered pr_referred pr_commenced pr_completed, format
...
forvalues year = `first_year'/`last_year' {
	...
	replace incident`year' = 1 if (start_fu <= `date') ///
									& (copd_prevalent_first <= `date') ///
									& (copd_prevalent_first > `previous_date') ///
									& (end_fu >= `date')
...
compress
save "`analysis_dir'/copd_pr_collapsed", replace


xpose, clear varname
order _varname

export excel "`analysis_dir'/outputs/copd_pr.xlsx", replace


use "`analysis_dir'/copd_pr_collapsed", clear  //leave on this one as easier to work with
