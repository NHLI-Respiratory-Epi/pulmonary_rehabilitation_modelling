//==============================================+
// COMPLETION IN 1 YEAR FOR THOSE WHO COMMENCED |
//==============================================+
...
/* Create log file */
capture log close
log using analysis_logs/LogisticRegression_completion, smcl replace
...
//Just keep patients that commenced PR
drop if pr_elig_commenced == .


//Remove patients with COMMENCED before start of follow-up
drop if pr_elig_commenced < start_fu

//Remove patients with less than a year follow-up after COMMENCED
drop if end_fu < (pr_elig_commenced + 365.25)


//Age at COMMENCED
gen age = year(pr_elig_commenced) - yob
summarize age, detail
order age, after(pr_elig_commenced)
...
//Outcome (within 1 year of COMMENCED)
gen byte completed = (pr_elig_completed <= (pr_elig_commenced + 365.25))
order completed, after(pr_elig_completed)
...
//Predictors
summarize age, detail
tab1 age_cat gender imd2015_5 ethnicity region
...
table (age_cat gender imd2015_5 ethnicity) (completed) (region), statistic(percent) //statistic(mean age)
...
//Nationally
logit completed i.age_cat i.gender i.imd2015_5 i.ethnicity, base
...
forvalues eng_region = 1/10 {
	
	display "{hline 100}"
	display "English region: `eng_region'"
	tab region if region == `eng_region'

	logit completed i.age_cat i.gender i.imd2015_5 i.ethnicity if region == `eng_region', base
}


log close
translate analysis_logs/LogisticRegression_completion.smcl ///
		  outputs/LogisticRegression_completion.pdf